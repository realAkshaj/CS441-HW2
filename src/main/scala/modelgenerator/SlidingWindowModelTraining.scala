...
//import org.deeplearning4j.optimize.api.{InvocationType, TrainingListener}
...
//    val batchSize = 32
//    val numEpochs = 5
////    val localModelPath = "src/main/finalOutput/model.zip" // Local path to save the model temporarily
////    val hdfsModelPath = "src/main/finalOutput/model.zip"
...
//    val localModelPath = "tmp/decoder_model.zip" // Local path to save the model temporarily
//    val hdfsModelPath = "hdfs://localhost:9000/output/user/aksha/sliding_window_dataset/decoder_model.zip"
//    val dataPath = "hdfs://localhost:9000/output/user/aksha/sliding_window_dataset/sliding_window_dataset.csv"
//    val metricsFilePath = "metrics_log.csv"
//
//    val metricsWriter = new PrintWriter(new BufferedWriter(new FileWriter(metricsFilePath, true)))
//    metricsWriter.write("Epoch,Iteration,Training Loss,Gradient Norm,Memory Usage (MB),Learning Rate,Accuracy,Duration (ms)\n")
//
//    // Helper function to parse a space-separated string of doubles
//    def parseDoubles(str: String): Array[Double] = {
//      str.replace("|", " ").split(" ").flatMap { s =>
//        try {
//          Some(s.toDouble)
//        } catch {
...
//      val learningRate = 0.001  // assuming fixed learning rate here
//
//      // Evaluate accuracy using DL4J Evaluation
//      val evaluation = new Evaluation()
//      trainingJavaRDD.foreach(dataSet => {
...
//      metricsWriter.write(s"$epoch,-1,$trainingLoss,-1,$memoryUsed,$learningRate,$accuracy,$epochDuration\n")
//      metricsWriter.flush()
//    }
//
//    // Ensure the directory for the local model path exists
...
//    val fs = FileSystem.get(new java.net.URI("hdfs://localhost:9000"), hadoopConfig)
//    fs.copyFromLocalFile(new Path(localModelPath), new Path(hdfsModelPath))
//    println(s"Model copied to HDFS at $hdfsModelPath")
//
//    // Clean up the local model file if necessary
//    //    new File(localModelPath).delete()
//
//    // Stop Spark Context
//    sc.stop()
//  }
//  // Custom listener for logging gradient norms
//  class GradientStatsListener(metricsWriter: PrintWriter) extends TrainingListener {
//
//    override def iterationDone(model: Model, iteration: Int, epoch: Int): Unit = {
//      if (model != null && model.gradient() != null) {
//        val norm = model.gradient().gradient().norm2Number()
//        println(s"Iteration $iteration - Epoch $epoch - Gradient Norm: $norm")
//        metricsWriter.write(s"$epoch,$iteration,-1,$norm,-1,-1,-1,-1\n")
//        metricsWriter.flush()
//      }
//    }
//
//    // Use java.util.Map and java.util.List instead of Scala types
...
object SlidingWindowModelTraining {
  ...
  def run(args: Array[String]): Unit = {
    ...
    val numEpochs = 5
    val localModelPath = "tmp/decoder_model.zip" // Local path to save the model temporarily
//    val hdfsModelPath = "hdfs:///user/akshaj/output/sliding_window_dataset/decoder_model.zip"
//    val dataPath = "hdfs:///user/akshaj/output/sliding_window_dataset/sliding_window_dataset.csv"
    val hdfsModelPath = "src/main/finalOutput/decoder_model.zip"
    val dataPath = "src/main/correctedInput/sliding_window_dataset/sliding_window_dataset.csv"
//    val hdfsModelPath = "hdfs://localhost:9000/output/user/aksha/sliding_window_dataset/decoder_model.zip"
//    val dataPath = "hdfs://localhost:9000/output/user/aksha/sliding_window_dataset/sliding_window_dataset.csv"
